...
def format_probability(value: float) -> str:
    """Format probability mass for compact table cells."""
...
def topk_rows(dataset: CountsDataset, qhats: Mapping[str, np.ndarray], *, k: int) -> pd.DataFrame:
    ...
    # Append summary rows following the historical paper-table contract.
...
def _emoji_summary_rows(
    ...
    """Return summary rows for mass and TV diagnostics in the Top-K table."""
    rows: list[dict[str, object]] = []

    # Top-K mass is computed on each estimator's own head set.
    mass_row: dict[str, object] = {"rank": "MassTopK"}
    for name in columns:
        mass_row[name] = format_probability(float(np.asarray(qhats[name])[top_indices[name]].sum()))
    rows.append(mass_row)

    # TV rows use either estimator-specific head sets or full vocabulary support.
    for reference in ("pooled", "unif_src"):
        head_row: dict[str, object] = {"rank": f"TV_to_{reference}_topK"}
        full_row: dict[str, object] = {"rank": f"TV_to_{reference}_all"}
        for name in columns:
            union = np.union1d(top_indices[reference], top_indices[name])
            head_row[name] = format_probability(0.5 * float(np.abs(qhats[reference][union] - qhats[name][union]).sum()))
            full_row[name] = format_probability(tv_between(qhats, reference, name))
        rows.extend([head_row, full_row])
    return rows
